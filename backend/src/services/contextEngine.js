 ...
 */
export async function buildContext(elderId, now = new Date()) {
...
export function decideOpening(context) {
  ...
  if (criticalOverdue) {
    return {
      ...
      speech: `${nama}, ${describeLate(criticalOverdue)} waktunya ${lower(criticalOverdue.title)}. Ini penting, sudah dilakukan belum?`,
  ...
  if (reminders.overdue.length > 0) {
    ...
    return {
      ...
      reminder: r,
      speech: `${nama}, ${describeLate(r)} waktunya ${lower(r.title)}. Sudah atau belum?`,
      expects: 'confirmation',
    };
  }
  ...
  if (reminders.dueNow.length > 0) {
    ...
    return {
      ...
      reminder: r,
      speech: `${nama}, sekarang waktunya ${lower(r.title)}.`,
      expects: 'confirmation',
    };
  }
  ...
  return {
    ...
    speech: `${greetingByHour(context.now)} ${nama}. Ada yang ingin diceritakan?`,
    expects: 'free',
  };
}
